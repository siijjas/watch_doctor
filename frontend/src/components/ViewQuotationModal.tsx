...
const getPrintUrl = (name: string) =>
  `/app/print/Quotation/${encodeURIComponent(name)}`;

const getPdfUrl = (name: string) =>
  `/api/method/frappe.utils.print_format.download_pdf?doctype=Quotation&name=${encodeURIComponent(name)}&format=Standard&no_letterhead=0`;
...
const statusColors: Record<string, string> = {
  ...
  Orderd: 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300',
...
export const ViewQuotationModal: React.FC<ViewQuotationModalProps> = ({
  ...
  const [editedValidTill, setEditedValidTill] = useState('');
  ...
  const fetchQuotation = useCallback(async () => {
    setIsLoading(true);
    try {
      const doc = await apiService.getDoc('Quotation', quotationName);
      setQuotation(doc);
      setEditedItems(JSON.parse(JSON.stringify(doc.items || [])));
      setEditedValidTill(doc.valid_till || '');
      setEditedTerms(doc.terms || '');
      const contactEmail = doc.contact_email || '';
      setEmailTo(contactEmail);
      setEmailSubject(`Quotation ${doc.name} – Watch Doctor`);
      setEmailMessage(
        `Dear ${doc.customer_name},\n\nPlease find your quotation details below.\n\nQuotation: ${doc.name}\nAmount: ${doc.currency || ''} ${Number(doc.grand_total || 0).toFixed(2)}\n\nBest regards,\nWatch Doctor`
      );
    } catch (err) {
  ...
  useEffect(() => {
    if (isOpen && quotationName) {
      setIsEditing(false);
      setShowEmailForm(false);
      fetchQuotation();
    }
  }, [isOpen, quotationName, fetchQuotation]);
  ...
  const handleCancelEdit = () => {
    ...
    setEditedValidTill(quotation.valid_till || '');
    setEditedTerms(quotation.terms || '');
    setIsEditing(false);
  };
    ...
    try {
      await apiService.apiFetchRaw('/api/method/frappe.core.doctype.communication.email.make', {
        ...
        body: JSON.stringify({
          ...
          attachments: JSON.stringify([{ print_format_attachment: 1, doctype: 'Quotation', name: quotationName, print_format: 'Standard' }]),
        }),
      });
  ...
  const taxTotal = (quotation?.taxes || []).reduce((s: number, t: any) => s + (parseFloat(t.tax_amount) || 0), 0);
  const displayItems = isEditing ? editedItems : (quotation?.items || []);
  const displayTotal = isEditing ? editedTotal : (quotation?.total || 0);
  const displayGrand = isEditing ? editedTotal + taxTotal : (quotation?.grand_total || 0);

  const docstatusVal = quotation?.docstatus ?? -1;
  const statusLabel = docstatusVal === 1 ? (quotation?.status || 'Submitted') : docstatusLabel[docstatusVal] || '';

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Quotation – ${quotationName}`}>
      {/* Toast */}
      {toast && (
      ...
      {isLoading ? (
        ...
        <p className="text-center text-gray-500 py-10">Could not load quotation.</p>
      ) : (
        <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(getPrintUrl(quotationName), '_blank')}
            >
              🖨️ Print
            </Button>
            <Button
              variant="outline"
              size="sm"
              onClick={() => window.open(getPdfUrl(quotationName), '_blank')}
            >
              📥 Download PDF
            </Button>
            <Button
          ...
          {showEmailForm && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-xl p-4 space-y-3">
              <h4 className="font-semibold text-blue-800 dark:text-blue-300">✉️ Send Quotation by Email</h4>
              <div>
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-5 space-y-4">
            <div className="flex justify-between items-start flex-wrap gap-3">
              <div>
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{quotation.name}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Quotation</p>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div>
                ...
                <p className="font-semibold text-gray-900 dark:text-white">{quotation.customer_name || quotation.customer}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Date</p>
                <p className="font-semibold text-gray-900 dark:text-white">{quotation.transaction_date || '—'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Valid Till</p>
                {isEditing ? (
                  <input
                    type="date"
                  ...
                  <p className="font-semibold text-gray-900 dark:text-white">{quotation.valid_till || '—'}</p>
                )}
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Currency</p>
                <p className="font-semibold text-gray-900 dark:text-white">{quotation.currency || '—'}</p>
              </div>
            </div>

            {/* Contact Info */}
            {(quotation.contact_display || quotation.contact_email || quotation.contact_mobile) && (
              <div className="border-t border-gray-200 dark:border-gray-700 pt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                {quotation.contact_display && (
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{quotation.contact_display}</p>
                  </div>
                )}
                {quotation.contact_email && (
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Email</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{quotation.contact_email}</p>
                  </div>
                )}
                {quotation.contact_mobile && (
                  <div>
                    <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Mobile</p>
                    <p className="text-sm font-medium text-gray-900 dark:text-white">{quotation.contact_mobile}</p>
                  </div>
                )}
              </div>
            )}
          ...
          {/* Watch / Repair Info (linked repair order) */}
          {quotation.items?.some((i: any) => i.item_name?.toLowerCase().includes('watch') || i.description) && (
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-2xl p-4">
              <p className="text-xs text-blue-600 dark:text-blue-400 uppercase tracking-wide font-semibold mb-1">Service Notes</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">
                {quotation.items.find((i: any) => i.description)?.description || 'Watch repair services'}
              </p>
            </div>
          )}
            ...
            <div>
              ...
              <div className="rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {(quotation.taxes || []).map((tax: any, idx: number) => (
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="text-gray-900 dark:text-white font-medium">
                  {quotation.currency} {Number(displayTotal).toFixed(2)}
                </span>
              </div>
              {taxTotal > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="text-gray-900 dark:text-white font-medium">
                    {quotation.currency} {Number(taxTotal).toFixed(2)}
                  </span>
                </div>
              )}
              {(quotation.discount_amount > 0 || quotation.additional_discount_percentage > 0) && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600 dark:text-gray-400">Discount</span>
                  <span className="text-red-600 dark:text-red-400 font-medium">
                    -{quotation.currency} {Number(quotation.discount_amount || 0).toFixed(2)}
                  </span>
                </div>
              )}
              <div className="flex justify-between text-lg font-bold border-t border-green-200 dark:border-green-700 pt-3 mt-2">
                <span className="text-gray-900 dark:text-white">Grand Total</span>
                <span className="text-green-700 dark:text-green-400">
                  {quotation.currency} {Number(displayGrand).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
